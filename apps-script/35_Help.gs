...
function buildHelp() {
  ...
  H('1. 처음 준비 (5분)');
  P('① 상단 [파일] > [사본 만들기] 로 내 드라이브에 복사합니다. 원본은 건드리지 않습니다.');
  P('② 메뉴 [' + APP.MENU + '] > ① 처음 설치 / 구조 복구');
  P('③ 메뉴 ② AI 연결 설정 — Gemini 키가 가장 부담이 적습니다(무료 등급 있음).');
  P('   구독 계정이면 빈 셀에 =AI("안녕") 을 넣어 보고, 되면 모델에서 Gemini(구독)을 고르면 키 없이 됩니다.');
  P('④ 👤 학생명단 에 반/번호/이름 입력 → 메뉴 ③ 학생 명단 동기화');
